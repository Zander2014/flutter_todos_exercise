...
class ThemeUtil{
  ...
  AppBarTheme getAppBarTheme(Color bgColor, Color iconColor){
    return AppBarTheme(
      ...
      color: bgColor,
      elevation: 0.0,
      textTheme: TextTheme(title: TextStyle(color: iconColor, fontSize: 20),),
    );
  }

  List<ThemeBean> defaultThemeBeans(BuildContext context) => [
    ThemeBean(themeName: IntlLocalizations.of(context).pink,
  ...
  Future<List<ThemeBean>> getThemeListWithCache(BuildContext context) async{
    ...
    for(var o in strings){
      ...
      ThemeBean themeBean = ThemeBean.fromJson(data);
      list.add(themeBean);
    }
